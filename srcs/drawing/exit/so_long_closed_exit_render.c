...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   so_long_closed_exit_render.c                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: gozsertt <gozsertt@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/08/05 19:23:30 by gozsertt          #+#    #+#             */
/*   Updated: 2021/08/07 11:27:01 by gozsertt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

static void	put_closed_exit_pixel(t_application *app, int x, int y,
	t_color color)
{
	t_image			*image;
	char			*tmp;
...
static void	draw_closed_exit_texture(t_engine *engine,
	float start_x, float start_y)
{
	...
	while (y < (engine->app->resolution.y / engine->map_size_y))
	{
		...
		while (x < (engine->app->resolution.x / engine->map_size_x))
		{
			...
			color = int_to_color(exit->pixels_closed[index]);
			put_closed_exit_pixel(engine->app,
...
void	closed_exit_render(t_engine *engine)
{
	...
	while (y < engine->map_size_y)
	{
		...
		while (x < engine->map_size_x)
		{
			line = list_at(map, y);
			if (line[x] == 'E')
				draw_closed_exit_texture(engine, x, y);
			x++;
		}
		y++;
	}
